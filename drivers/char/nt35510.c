		...
		size_t size, loff_t *poss)
{
	...
	// //show nt35510_drvdata
	// printk(KERN_ALERT "nt35510_write: xres=%d, yres=%d\n", drvdata->xres, drvdata->yres);
	// printk(KERN_ALERT "nt35510_write: curr_off=%lld\n", drvdata->curr_off);
	// printk(KERN_ALERT "nt35510_write: f_pos=%lld\n", file->f_pos);
	// printk(KERN_ALERT "nt35510_write: size=%d\n", size);
	...
	if (!data) {
		ret = -ENOMEM;
		printk(KERN_ALERT "nt35510_write: memory allocation failed\n");
		return ret;
	}
	...
	// printk(KERN_ALERT "nt35510_write: ret=%d\n", ret);
	return ret;
}
